...
- 네이버페이 증권 종목토론실: 대표 대형주/테마주 토론글 목록 수집
- 뽐뿌/FMKorea/코인판/MLB파크 공개 게시판 목록 수집
- SQLite 저장
- 사전 기반 감성 점수화
- 처음 발견/마지막 발견 날짜 추적
- 네이버 카페/종토방/뽐뿌/FMKorea/코인판/MLB파크 글번호 파싱 후 소스별 이전 최고 글번호보다 큰 URL을 신규 포착으로 판정
...
새 공개 커뮤니티 소스를 잠시 빼고 기존 네이버/디시/보배드림만 보려면:

```bash
python3 -m sentiment_index.cli collect --no-community
```
...
- 네이버페이 증권 종목토론실: 삼성전자, SK하이닉스 등 대표 종목의 개인투자자 반응 표본. 기본 가중치 `0.8~1.0`
- 뽐뿌 증권포럼: 국내 개인투자자 재테크/주식 토론 보강 표본. 기본 가중치 `0.8`
- FM코리아 주식/코인 게시판: 비교적 빠른 온라인 투자 관심 변화 보강 표본. 기본 가중치 `0.7`
- 코인판 자유게시판/코인정보: 코인 전용 커뮤니티 표본. 기본 가중치 `0.9`
- MLB파크 불펜: 대중 잡담 속 코인/주식 언급 보완 표본. 키워드 필터 적용, 기본 가중치 `0.6`

네이버 카페/데이터랩을 제외한 공개 커뮤니티 소스는 공개 목록 HTML에서 제목/URL 중심으로 수집합니다. 일반 커뮤니티 특성상 노이즈가 많으므로, 대시보드의 `Source Breakdown`과 `spam_rate`를 같이 보면서 가중치를 조정합니다. 소스 추가는 기존 과거 데이터를 다시 쓰지 않고, 추가된 시점 이후 수집분부터 `daily_snapshots`/`hourly_snapshots`에 반영됩니다.
...
- 종토방/뽐뿌/FMKorea/코인판/MLB파크는 공식 API가 아니라 공개 HTML 파싱이므로 사이트 구조가 바뀌면 수집기가 빈 결과를 낼 수 있습니다. 이 경우 `Source Breakdown`에서 특정 소스 글 수가 갑자기 0이 되는지 확인합니다.
